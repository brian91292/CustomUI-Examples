...
namespace CustomUI_Examples
{
    class MenuCreation : MonoBehaviour
    {
        public static void CreateNewMenu()
        {
            ...
            var menu = BeatSaberUI.CreateCustomMenu<CustomMenu>("Testing New Menu"); //Required, the string here is the name that will appear at the top of the view controller
            var viewController = BeatSaberUI.CreateViewController<CustomViewController>(); //Required
            menu.SetMainViewController(viewController, true, (firstActivation, type) => //Required
            {
